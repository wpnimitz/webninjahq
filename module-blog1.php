...
class fxp_module_blog1 extends ET_Builder_Module {
	function init() {
		$this->name       = esc_html__( 'Module Blog 1', 'et_builder' );
		$this->slug       = 'et_pb_fxp_module_blog1';
		...
		$this->fields_defaults = array(
			...
			'columns_number'    => array( 1, 'add_default_setting' ),
			'meta_date'         => array( 'M j, Y', 'add_default_setting' ),
			'show_thumbnail'    => array( 'on' ),
			'show_content'      => array( 'off' ),
			'show_more'         => array( 'off' ),
			'button_text'    	=> array( 'Learn More', 'add_default_setting' ),
	...
	function get_fields() {
		$fields = array(
			...
			'columns_number' => array(
				'label'             => esc_html__( 'Post Per Row', 'et_builder' ),
				'type'              => 'range',
				...
				'option_category'   => 'configuration',
				'description'       => esc_html__( 'Choose how many posts per row you would like to display.', 'et_builder' ),
				'computed_affects'   => array(
					'__posts',
				),
			),
			...
			'button_text' => array(
				...
				'type'              => 'text',
				'option_category'   => 'configuration',
				'description'       => esc_html__( 'Type the desire button text.', 'et_builder' ),
				'computed_affects'   => array(
					'__posts',
				),
			),
		...
		if ( $query->have_posts() ) {
			...
			while( $query->have_posts() ) {
				...
				?>
					...
					<div <?php post_class( 'inner clearfix' . $no_thumb_class . $overlay_class ) ?>>
                <?php
				et_divi_post_format_content();
				if ( ! in_array( $post_format, array( 'link', 'audio', 'quote' ) ) ) {
					if ( 'video' === $post_format && false !== ( $first_video = et_get_first_video() ) ) :
					...
					elseif ( '' !== $thumb && 'on' === $show_thumbnail ) :
						if ( 'on' !== $fullwidth ); ?>
							...
							<img src="<?php echo $thumbnail["thumb"]; ?>" alt="service">
                ...
                <div class="blog1-content">
                    <h5><a href="<?php echo $post_url; ?>"><?php echo $titletext; ?></a></h5>
						<?php
							if ( 'on' === $show_author || 'on' === $show_date || 'on' === $show_categories || 'on' === $show_comments ) {
								printf( '<p class="post_item-meta">%1$s %2$s %3$s %4$s %5$s %6$s %7$s</p>',
									(
										'on' === $show_author
											? et_get_safe_localization( sprintf( __( '%s', 'et_builder' ), et_pb_get_the_author_posts_link() ) )
											: ''
									),
									(
									...
									(
										'on' === $show_date
											? et_get_safe_localization( sprintf( __( '%s', 'et_builder' ), '<time>' . esc_html( get_the_date( $meta_date ) ) . '</time>' ) )
											: ''
									),
									(
					...
					<?php
						if ( 'on' !== $show_content ) {
							$more = 'on' == $show_more ? sprintf( ' <a href="%1$s" class="blog1-morelink" >%2$s</a>' , esc_url( get_permalink() ), esc_html__( $button_text, 'et_builder' ) )  : '';
						echo $more;
						}
					?>
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		$columns_number      = $this->shortcode_atts['columns_number'];
		...
		if ( have_posts() ) {
			while ( have_posts() ) {
				...
				$titletext = get_the_title();
				$content = $this->my_content(get_the_content());
	...
	<?php
		...
		$output = sprintf(
			'<div%5$s class="%1$s%3$s%6$s blog1_column-%8$s">
				%2$s
			%4$s',
